...
use synth_core::models::banking::{RiskTier, SourceOfFunds, TurnoverBand};
use uuid::Uuid;
...
impl CustomerLabel {
    ...
    fn customer_type_risk_weight(
        customer_type: &synth_core::models::banking::BankingCustomerType,
    ) -> f64 {
        use synth_core::models::banking::BankingCustomerType;
        match customer_type {
...
impl AccountLabel {
    ...
    fn estimate_tx_count(
        account_type: &synth_core::models::banking::BankAccountType,
    ) -> u32 {
        use synth_core::models::banking::BankAccountType;
...
impl EntityLabelExtractor {
    ...
    pub fn extract_customers(customers: &[BankingCustomer]) -> Vec<CustomerLabel> {
        customers
            .iter()
            .map(CustomerLabel::from_customer)
            .collect()
    }
