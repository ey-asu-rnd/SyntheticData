...
use crate::models::{AmlScenario, CaseNarrative, CaseRecommendation, RedFlag, RedFlagCategory, ViolatedExpectation};
...
impl NarrativeGenerator {
    ...
    fn generate_evidence_points(&mut self, scenario: &AmlScenario) -> Vec<String> {
        ...
        if matches!(scenario.sophistication, Sophistication::Professional | Sophistication::Advanced | Sophistication::StateLevel) {
            points.push("Use of intermediary entities to obscure beneficial ownership".to_string());
        }
        if matches!(scenario.sophistication, Sophistication::Advanced | Sophistication::StateLevel) {
            points.push(
    ...
    fn generate_violated_expectations(&mut self, scenario: &AmlScenario) -> Vec<ViolatedExpectation> {
    ...
    fn recommend_action(&self, scenario: &AmlScenario) -> CaseRecommendation {
        ...
        if matches!(
            scenario.typology,
            AmlTypology::SanctionsEvasion | AmlTypology::TerroristFinancing | AmlTypology::Corruption
        ) {
