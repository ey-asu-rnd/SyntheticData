...
impl TransactionGenerator {
    ...
    pub fn new(config: BankingConfig, seed: u64) -> Self {
        ...
        Self {
            ...
            uuid_factory: DeterministicUuidFactory::new(seed, synth_core::GeneratorType::JournalEntry),
    ...
    pub fn generate_account_transactions(
        ...
        while current_date <= self.end_date {
            ...
            let daily_txns = self.generate_daily_transactions(customer, account, current_date, &mut balance);
    ...
    fn generate_daily_transactions(
        ...
        if self.should_generate_recurring(customer, date) {
            transactions.extend(self.generate_recurring_transactions(customer, account, date, balance));
        }
        ...
        for _ in 0..discretionary_count {
            if let Some(txn) = self.generate_discretionary_transaction(customer, account, date, balance) {
                transactions.push(txn);
            }
        }
    ...
    fn calculate_daily_transaction_count(&mut self, customer: &BankingCustomer, date: NaiveDate) -> u32 {
        ...
        let (amount, category, counterparty) = match &customer.persona {
            Some(PersonaVariant::Retail(p)) => {
                ...
                let category = match p {
                    synth_core::models::banking::RetailPersona::Retiree => TransactionCategory::Pension,
                    synth_core::models::banking::RetailPersona::GigWorker => TransactionCategory::FreelanceIncome,
    ...
    fn generate_recurring_transactions(
        ...
        for (category, min, max, probability) in recurring_types {
            if self.rng.gen::<f64>() < probability {
                let amount = Decimal::from_f64_retain(self.rng.gen_range(min..max))
                    .unwrap_or(Decimal::ZERO);
    ...
    fn select_category(&mut self, channel: TransactionChannel) -> (TransactionCategory, Option<MerchantCategoryCode>) {
        let categories: Vec<(TransactionCategory, Option<MerchantCategoryCode>, f64)> = match channel {
            TransactionChannel::CardPresent | TransactionChannel::CardNotPresent => vec![
                (TransactionCategory::Groceries, Some(MerchantCategoryCode::GROCERY_STORES), 0.25),
                (TransactionCategory::Dining, Some(MerchantCategoryCode::RESTAURANTS), 0.20),
                (TransactionCategory::Shopping, Some(MerchantCategoryCode::DEPARTMENT_STORES), 0.20),
                (TransactionCategory::Transportation, Some(MerchantCategoryCode::GAS_STATIONS), 0.15),
                (TransactionCategory::Entertainment, None, 0.10),
                (TransactionCategory::Healthcare, Some(MerchantCategoryCode::MEDICAL), 0.05),
                (TransactionCategory::Other, None, 0.05),
            ],
            TransactionChannel::Atm => vec![
                (TransactionCategory::AtmWithdrawal, None, 1.0),
            ],
            TransactionChannel::PeerToPeer => vec![
                (TransactionCategory::P2PPayment, None, 1.0),
            ],
            _ => vec![
    ...
    fn select_counterparty(&mut self, category: TransactionCategory) -> CounterpartyRef {
        match category {
            TransactionCategory::AtmWithdrawal => {
                CounterpartyRef::atm("Branch ATM")
            }
            TransactionCategory::P2PPayment => {
                CounterpartyRef::peer("Friend", None)
            }
            _ => {
                self.counterparty_pool.merchants
                    .choose(&mut self.rng)
