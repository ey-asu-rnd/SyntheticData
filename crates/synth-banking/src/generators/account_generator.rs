...
use synth_core::models::banking::{AccountFeatures, BankAccountType, BankingCustomerType};
...
impl AccountGenerator {
    ...
    pub fn new(config: BankingConfig, seed: u64) -> Self {
        Self {
            ...
            uuid_factory: DeterministicUuidFactory::new(seed, synth_core::GeneratorType::ARSubledger), // Reuse for banking accounts
            account_counter: 0,
        }
    }
    ...
    fn determine_account_count(&mut self, customer: &BankingCustomer) -> u32 {
        ...
        let multiplier = match &customer.persona {
            ...
            Some(PersonaVariant::Business(p)) => {
                use synth_core::models::banking::BusinessPersona;
                match p {
    ...
    fn generate_secondary_account(&mut self, customer: &BankingCustomer, index: u32) -> BankAccount {
        let account_id = self.uuid_factory.next();
        let account_number = self.generate_account_number();
        ...
        // Set initial balance
        account.current_balance = self.generate_initial_balance(customer) * Decimal::from_f64_retain(0.3).unwrap_or(Decimal::ZERO);
        account.available_balance = account.current_balance;
    ...
    fn select_secondary_account_type(&mut self, customer: &BankingCustomer, _index: u32) -> BankAccountType {
    ...
    fn generate_features(&mut self, customer: &BankingCustomer, is_primary: bool) -> AccountFeatures {
        let mut features = match customer.customer_type {
            BankingCustomerType::Retail if is_primary => {
                if matches!(customer.persona, Some(PersonaVariant::Retail(synth_core::models::banking::RetailPersona::HighNetWorth))) {
    ...
    fn generate_routing_number(&mut self) -> String {
        let routing_prefixes = ["021", "026", "031", "041", "051", "061", "071", "081", "091"];
