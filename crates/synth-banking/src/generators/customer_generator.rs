...
use synth_core::models::banking::{BankingCustomerType, BusinessPersona, RetailPersona, RiskTier, TrustPersona};
use synth_core::DeterministicUuidFactory;
use uuid::Uuid;

use crate::config::{BankingConfig, RiskAppetite};
use crate::models::{
    BankingCustomer, CustomerName, KycProfile, PepCategory, PersonaVariant,
};
...
impl CustomerGenerator {
    ...
    pub fn new(config: BankingConfig, seed: u64) -> Self {
        ...
        Self {
            ...
            uuid_factory: DeterministicUuidFactory::new(seed, synth_core::GeneratorType::Customer),
            start_date,
            end_date,
        }
    }
    ...
    pub fn generate_business_customer(&mut self) -> BankingCustomer {
        ...
        let mut customer = BankingCustomer::new_business(
            customer_id,
            &name,
            &country,
            onboarding_date,
    ...
    pub fn generate_trust_customer(&mut self) -> BankingCustomer {
        ...
        customer.kyc_profile = KycProfile::high_net_worth()
            .with_turnover(synth_core::models::banking::TurnoverBand::VeryHigh);
    ...
    fn generate_business_name(&mut self, persona: BusinessPersona) -> String {
        let prefixes = ["Acme", "Global", "Premier", "Advanced", "Pacific", "Summit", "Atlas", "Apex"];
        let industries = match persona {
            ...
            BusinessPersona::ImportExport => ["Trading", "Import Export", "Commerce", "International"],
            BusinessPersona::ProfessionalServices => ["Consulting", "Advisors", "Partners", "Associates"],
    ...
    fn calculate_retail_risk_tier(&mut self, persona: RetailPersona, country: &str) -> RiskTier {
        ...
        RiskTier::from_score(score.max(0.0).min(100.0) as u8)
    }

    /// Calculate risk tier for business customer.
    fn calculate_business_risk_tier(&mut self, persona: BusinessPersona, country: &str) -> RiskTier {
        let base_score = persona.base_risk_score();
        let mut score = base_score as f64 * 10.0;
    ...
    fn generate_retail_kyc_profile(&mut self, persona: RetailPersona) -> KycProfile {
        use synth_core::models::banking::{FrequencyBand, SourceOfFunds, TurnoverBand, CashIntensity};
        ...
        let source = match persona {
            ...
            RetailPersona::GigWorker | RetailPersona::SeasonalWorker => SourceOfFunds::SelfEmployment,
    ...
    fn generate_business_kyc_profile(&mut self, persona: BusinessPersona) -> KycProfile {
        ...
        let (cash_min, cash_max) = persona.cash_deposit_frequency();
    ...
    fn generate_email(&mut self, first: &str, last: &str) -> String {
        ...
        format!("{}.{}{}@{}", first.to_lowercase(), last.to_lowercase(), num, domain)
    }

    /// Generate phone number.
    fn generate_phone(&self, country: &str) -> String {
        match country {
            "US" | "CA" => format!("+1-555-{:03}-{:04}", rand::random::<u16>() % 1000, rand::random::<u16>() % 10000),
            "GB" => format!("+44-7{:03}-{:06}", rand::random::<u16>() % 1000, rand::random::<u32>() % 1000000),
            _ => format!("+{}-{:010}", rand::random::<u8>() % 90 + 10, rand::random::<u64>() % 10000000000),
        }
    }
...
mod tests {
    ...
    fn test_customer_generation() {
        ...
        let retail_count = customers.iter().filter(|c| c.customer_type == BankingCustomerType::Retail).count();
        let business_count = customers.iter().filter(|c| c.customer_type == BankingCustomerType::Business).count();
