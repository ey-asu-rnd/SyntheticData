...
use synth_core::models::banking::{AmlTypology, LaunderingStage, Sophistication};
...
impl TypologyInjector {
    ...
    pub fn inject(
        ...
        let suspicious_indices: Vec<usize> =
            customer_indices.into_iter().take(suspicious_count).collect();
    ...
    fn select_typology(&mut self) -> AmlTypology {
        let rates = [
            ...
            (AmlTypology::FunnelAccount, self.config.typologies.funnel_rate),
            (AmlTypology::Layering, self.config.typologies.layering_rate),
            (AmlTypology::MoneyMule, self.config.typologies.mule_rate),
            (AmlTypology::AccountTakeover, self.config.typologies.fraud_rate * 0.3),
            (AmlTypology::FirstPartyFraud, self.config.typologies.fraud_rate * 0.3),
            (AmlTypology::AuthorizedPushPayment, self.config.typologies.fraud_rate * 0.4),
        ];
    ...
    fn inject_structuring(
        ...
        if let Some(account) = account {
            ...
            let start_date = NaiveDate::parse_from_str(&self.config.population.start_date, "%Y-%m-%d")
    ...
    fn apply_spoofing(
        ...
        for txn in transactions.iter_mut() {
            if txn.is_suspicious {
                // Find the customer
                let customer = customers.iter().find(|c| c.account_ids.contains(&txn.account_id));
