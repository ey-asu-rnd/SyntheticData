...
impl FraudInjector {
    ...
    pub fn new(seed: u64) -> Self {
        Self {
            ...
            uuid_factory: DeterministicUuidFactory::new(seed, synth_core::GeneratorType::Anomaly),
        }
    }
    ...
    pub fn generate_account_takeover(
        ...
        for i in 0..extractions {
            ...
            let timestamp = takeover_date
                .and_hms_opt(current_hour as u32, self.rng.gen_range(0..60), self.rng.gen_range(0..60))
        ...
        if matches!(sophistication, Sophistication::Professional | Sophistication::Advanced | Sophistication::StateLevel) {
    ...
    pub fn generate_fake_vendor(
        ...
        for i in 0..payments {
            ...
            let invoice_ref = format!("INV-{:04}-{:06}", current_date.year() % 100, self.rng.gen::<u32>() % 1_000_000);
            ...
            current_date = current_date + chrono::Duration::days(interval);
    ...
    pub fn generate_bec(
        ...
        for i in 0..payments {
            ...
            current_date = current_date + chrono::Duration::days(self.rng.gen_range(1..3));
        }
    ...
    fn random_ato_extraction(&mut self, index: usize) -> (TransactionChannel, TransactionCategory, CounterpartyRef, String) {
        let extractions = [
            (TransactionChannel::Wire, TransactionCategory::TransferOut, CounterpartyRef::person("External Account"), "External transfer".to_string()),
            (TransactionChannel::Ach, TransactionCategory::TransferOut, CounterpartyRef::person("Linked Account"), "ACH transfer out".to_string()),
            (TransactionChannel::CardNotPresent, TransactionCategory::Shopping, CounterpartyRef::merchant_by_name("Online Store", "5999"), "Online purchase".to_string()),
            (TransactionChannel::Atm, TransactionCategory::AtmWithdrawal, CounterpartyRef::atm("ATM"), "ATM withdrawal".to_string()),
            (TransactionChannel::CardNotPresent, TransactionCategory::Shopping, CounterpartyRef::merchant_by_name("Gift Card Vendor", "5815"), "Gift card purchase".to_string()),
        ];
    ...
    fn random_bec_recipient(&mut self) -> (String, String) {
        let recipients = [
            ("International Trade Co Ltd", "URGENT: Updated payment details - Invoice payment"),
            ("Overseas Partner Holdings", "Wire transfer - NEW BANK DETAILS"),
            ("Foreign Supplier Pte Ltd", "Payment for goods - UPDATED ACCOUNT"),
            ("Global Trading Services", "URGENT: Supplier payment - new instructions"),
            ("Asian Manufacturing Ltd", "Invoice settlement - REVISED BANK"),
        ];
...
mod tests {
    ...
    fn create_test_account(customer: &BankingCustomer) -> BankAccount {
        BankAccount::new(
            Uuid::new_v4(),
            "****1234".to_string(),
            synth_core::models::banking::BankAccountType::Checking,
    ...
    fn test_account_takeover_generation() {
        ...
        let transactions =
            injector.generate_account_takeover(&customer, &account, start, end, Sophistication::Standard);
    ...
    fn test_fake_vendor_generation() {
        ...
        let transactions =
            injector.generate_fake_vendor(&customer, &account, start, end, Sophistication::Professional);
    ...
    fn test_bec_generation() {
        ...
        for txn in &transactions {
            assert!(txn.is_suspicious);
            assert_eq!(txn.suspicion_reason, Some(AmlTypology::BusinessEmailCompromise));
    ...
    fn test_app_fraud_generation() {
        ...
        for txn in &transactions {
            assert!(txn.is_suspicious);
            assert_eq!(txn.suspicion_reason, Some(AmlTypology::AuthorizedPushPayment));
        }
    }
}
