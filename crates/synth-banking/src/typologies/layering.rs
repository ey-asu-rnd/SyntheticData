...
impl LayeringInjector {
    ...
    pub fn generate(
        ...
        for layer in 0..layers {
            ...
            current_date = current_date + chrono::Duration::days(jitter);
            ...
            let num_slices = if matches!(sophistication, Sophistication::Professional | Sophistication::Advanced | Sophistication::StateLevel) {
            ...
            for slice in 0..num_slices {
                let slice_amount = if slice == num_slices - 1 {
                    ...
                    self.rng.gen_range((portion - variance)..(portion + variance))
                };
    ...
    fn random_layer_destination(&mut self, layer: usize) -> (TransactionChannel, String) {
        let destinations = [
            (TransactionChannel::Wire, format!("Offshore Holdings {}", layer + 1)),
            (TransactionChannel::Ach, format!("Investment Co {}", layer + 1)),
            (TransactionChannel::Swift, format!("Trade Finance {} Ltd", layer + 1)),
            (TransactionChannel::Wire, format!("Consulting {} LLC", layer + 1)),
        ];
...
mod tests {
    ...
    fn test_layering_generation() {
        ...
        let transactions =
            injector.generate(&customer, &account, start, end, Sophistication::Professional);
