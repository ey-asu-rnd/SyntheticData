...
use synth_core::models::banking::TransactionCategory;
...
impl SpoofingEngine {
    ...
    fn spoof_timing(&mut self, txn: &mut BankTransaction, _customer: &BankingCustomer) {
        ...
        if current_hour < 7 || current_hour > 22 {
            ...
            if let Some(new_time) = txn.timestamp_initiated.date_naive().and_hms_opt(new_hour, new_minute, new_second) {
                txn.timestamp_initiated = chrono::DateTime::<chrono::Utc>::from_naive_utc_and_offset(new_time, chrono::Utc);
            }
        }
    }
    ...
    fn add_timing_jitter(&mut self, txn: &mut BankTransaction) {
        ...
        txn.timestamp_initiated = txn.timestamp_initiated + chrono::Duration::minutes(jitter_minutes as i64);
    }
    ...
    pub fn calculate_effectiveness(&self, txn: &BankTransaction, customer: &BankingCustomer) -> f64 {
        ...
        // Not near thresholds
        if amount < 9_000.0 || amount > 11_000.0 {
            score += 0.5;
        }
        factors += 1;
...
mod tests {
    ...
    fn test_spoofing_engine() {
        ...
        let mut txn = BankTransaction::new(
            ...
            Decimal::from(9999),
            "USD",
            synth_core::models::banking::Direction::Outbound,
            synth_core::models::banking::TransactionChannel::Wire,
            TransactionCategory::TransferOut,
