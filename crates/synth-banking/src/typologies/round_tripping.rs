...
impl RoundTrippingInjector {
    ...
    pub fn generate(
        ...
        for trip in 0..trips {
            ...
            current_date = current_date + chrono::Duration::days(gap);
    ...
    fn add_intermediate_transactions(
        ...
        for i in 0..num_intermediate {
            let day_offset = self.rng.gen_range(1..available_days) as i64;
...
mod tests {
    ...
    fn test_round_tripping_has_both_directions() {
        ...
        let account = BankAccount::new(
            Uuid::new_v4(),
            "****5678".to_string(),
            synth_core::models::banking::BankAccountType::BusinessOperating,
            customer.customer_id,
            "USD",
            NaiveDate::from_ymd_opt(2024, 1, 1).unwrap(),
        );
        ...
        let has_outbound = transactions.iter().any(|t| t.direction == Direction::Outbound);
        let has_inbound = transactions.iter().any(|t| t.direction == Direction::Inbound);
