...
use synth_core::models::{
    AssetAccountDetermination, AssetClass, AssetStatus, DepreciationMethod,
    FixedAsset, FixedAssetPool,
};
...
impl AssetGenerator {
    ...
    pub fn generate_asset(
        ...
        if self.rng.gen::<f64>() < self.config.disposed_rate {
            let disposal_date = acquisition_date + chrono::Duration::days(
    ...
    pub fn generate_diverse_pool(
        ...
        let class_counts = [
            ...
            (AssetClass::LeaseholdImprovements, (count as f64 * 0.05) as usize),
        ];
    ...
    fn select_depreciation_method(&mut self, asset_class: &AssetClass) -> DepreciationMethod {
        ...
        if matches!(asset_class, AssetClass::Land | AssetClass::ConstructionInProgress) {
    ...
    fn generate_acquisition_cost_for_class(&mut self, asset_class: &AssetClass) -> Decimal {
        let (min, max) = match asset_class {
            ...
            AssetClass::ConstructionInProgress => (Decimal::from(100_000), Decimal::from(2_000_000)),
        };
    ...
    fn generate_account_determination(&self, asset_class: &AssetClass) -> AssetAccountDetermination {
        match asset_class {
            AssetClass::Buildings | AssetClass::BuildingImprovements => {
...
mod tests {
    ...
    fn test_asset_generation() {
        ...
        assert!(asset.useful_life_months > 0 || matches!(asset.asset_class, AssetClass::Land | AssetClass::ConstructionInProgress));
    }
    ...
    fn test_diverse_pool() {
        ...
        let machinery_count = pool.assets.iter()
            .filter(|a| a.asset_class == AssetClass::Machinery)
            .count();
        let it_count = pool.assets.iter()
    ...
    fn test_deterministic_generation() {
        ...
        let asset1 = gen1.generate_asset(
            "1000",
            NaiveDate::from_ymd_opt(2024, 1, 1).unwrap(),
        );
        let asset2 = gen2.generate_asset(
            "1000",
            NaiveDate::from_ymd_opt(2024, 1, 1).unwrap(),
        );
