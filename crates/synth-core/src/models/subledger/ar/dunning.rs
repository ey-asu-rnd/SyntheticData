...
impl DunningLetter {
    ...
    fn recalculate_totals(&mut self) {
        self.total_amount_due = self.total_dunned_amount + self.dunning_charges + self.interest_amount;
    }
...
impl CustomerDunningSummary {
    ...
    pub fn from_letters(
        ...
        let total_charges: Decimal = customer_letters
            .iter()
            .map(|l| l.dunning_charges)
            .sum();

        let total_interest: Decimal = customer_letters
            .iter()
            .map(|l| l.interest_amount)
            .sum();

        let last_dunning_date = customer_letters
            .iter()
            .map(|l| l.dunning_date)
            .max();
