...
use synth_banking::models::{BankAccount, BankTransaction, BankingCustomer, CounterpartyPool};
use synth_core::models::banking::{AmlTypology, Direction, RiskTier};
...
impl BankingGraphBuilder {
    ...
    pub fn add_accounts(&mut self, accounts: &[BankAccount], customers: &[BankingCustomer]) {
        ...
        let customer_map: HashMap<_, _> = customers
            .iter()
        ...
        for account in accounts {
            ...
            if let Some(customer) = customer_map.get(&account.primary_owner_id) {
                ...
                let edge = GraphEdge::new(0, customer_id, account_id, EdgeType::Ownership)
                    .with_weight(1.0)
                    .with_property("relationship", crate::models::EdgeProperty::String("account_owner".to_string()));
    ...
    fn add_account(&mut self, account: &BankAccount) -> NodeId {
        ...
        if self.config.include_risk_features {
            ...
            // Has debit card
            node.features.push(if account.features.debit_card { 1.0 } else { 0.0 });

            // Has international capability
            node.features.push(if account.features.international_transfers { 1.0 } else { 0.0 });
        }
    ...
    pub fn add_counterparties(&mut self, pool: &CounterpartyPool) {
        ...
        for merchant in &pool.merchants {
            self.add_counterparty_node(&merchant.name, "merchant", Some(&format!("{:?}", merchant.mcc)));
        }
        ...
        for utility in &pool.utilities {
            self.add_counterparty_node(&utility.name, "utility", Some(&format!("{:?}", utility.utility_type)));
        }
    }

    /// Adds a counterparty node.
    fn add_counterparty_node(&mut self, name: &str, cp_type: &str, category: Option<&str>) -> NodeId {
    ...
    fn create_transaction_edge(
        ...
        if self.config.include_risk_features {
            ...
            // Is cross-border
            edge.features.push(if txn.is_cross_border() { 1.0 } else { 0.0 });

            // Risk score from transaction
            edge.features.push(txn.calculate_risk_score() as f64 / 100.0);
        }

        // Ground truth labels
        if txn.is_suspicious {
            edge = edge.as_anomaly(&format!("{:?}", txn.suspicion_reason.unwrap_or(AmlTypology::Structuring)));
    ...
    fn aggregate_transaction_edge(&mut self, source: NodeId, target: NodeId, txn: &BankTransaction) {
        let key = (source, target);
        let amount: f64 = txn.amount.try_into().unwrap_or(0.0);
        let date = txn.timestamp_initiated.date_naive();

        let agg = self.edge_aggregation.entry(key).or_insert(AggregatedBankingEdge {
            source,
            target,
            total_amount: 0.0,
            count: 0,
    ...
    pub fn build(mut self) -> Graph {
        ...
        if self.config.aggregate_parallel_edges {
            for ((source, target), agg) in self.edge_aggregation {
                ...
                edge.features.push(agg.suspicious_count as f64 / agg.count.max(1) as f64);

                let duration = (agg.last_date - agg.first_date).num_days() as f64;
                edge.features.push(duration);

                // Average amount per transaction
                edge.features.push(agg.total_amount / agg.count.max(1) as f64);
...
mod tests {
    ...
    use uuid::Uuid;
    use synth_core::models::banking::{BankAccountType, TransactionCategory, TransactionChannel};
    use synth_banking::models::CounterpartyRef;
